...
$id=$_POST['id'];
	...
	$fetch_res_p = $mysqli->query("select * from attendance where member_id='$id' and date='$date'") or die(mysql_error());
		...
		$pm_late_under=((($h)-($pm_hour_end))*(60))-($i);
...
$up_emp=("update attendance set date='$date', am_out='$time',undertime='$am_late_under' where   member_id='$member_id_p' and date='$date' ");
 $mysqli->query($up_emp);
...
else if( $row_count=='1' && $am_out!=null &&  $a =='pm'&&  $h =='12')
{
...
else if( $row_count==null && $am_out==null &&  $a =='pm'&&  $h =='12')
{
